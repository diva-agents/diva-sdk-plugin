...
| `knowledge` | `str \| None` | `None` | Not yet wired. Passing anything raises `DivaNotImplementedError` immediately at construction: `"knowledge/RAG is not wired in the thin client yet"`. The field exists so the surface is stable. |
