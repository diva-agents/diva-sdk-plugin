...
2. **This plugin's bundled platform MCP** (server `platform` in the plugin's
   `.mcp.json`, `https://mcp.diva-ai.ru/mcp`, bearer-authed via the
   `diva_api_key` plugin setting). This is a set of tools available to *you*
   (the assistant) for operating agents/channels/CRM/sessions on the platform —
